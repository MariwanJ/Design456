...
#TODO: FIXME:
def draw_2Darrow(Points=[], color=FR_COLOR.FR_BLACK,scale=1,type=0, rotation=[1.0, 1.0, 1.0, 0.0]):

    root=coin.SoSeparator()
    arrShape= coin.SoVRMLShape()
    arrAppearance= coin.SoAppearnace()
    arrMaterial = coin.SoVRMLMaterial()
    arrvrmlInd= coin.SoVRMLIndexedFaceSet()
    arrvrmlCor= coin.SoVRMLCoordinate()
    colorVert= coin.SocolorPerVertex()

    if type==0:
        arrvrmlCor.point=[ {84.044998 ,-56.896271, 0},
    ...
    colorVert.coordIndex=[  27, 25, 26, -1, 23, 25, 27, -1,
