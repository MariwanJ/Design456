...
__updated__ = '2022-01-24 22:04:31'
    ...
    def Activated(self):            
        try:
            ...
            if len(self.selectedObj) == 1:
                ...
                if not(self.selectedObj[0].HasSubObjects):
                    #Only One object that doesn't have subobjects
                    if not isinstance(self.selectedObj[0].Object.Shape, Part.Face):
                        AllObjects=[self.ExtractThenExtrude()]
                    else: 
                        # Here it can be a face with multiple faces
                        # Or a 3D object with multiple faces?
                        AllObjects=[self.selectedObj[0].Object]
                else:
                    AllObjects=self.selectedObj.SubObjects
            else:
                allObjects=self.selectedObj

    def ExtrudeFace(self, allObjects):
        try:
                App.ActiveDocument.openTransaction(
                    translate("Design456", "Extrude"))
                m = self.selectedObj[0].Object
