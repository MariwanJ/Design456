...
#  FaceWireArray.py
#  
#  Copyright 2021 Jaise James <jaisekjames at gmail dot com>
#  
...
def faceArray(tool, tool_face, base_faces, skip_edges, offset) :
  ...
  for base_face in base_faces :
    ...
    for wire in wire_list :
      ...
      tool_tran = transform_tool(tool_copy, base_face, tool_face, offsetPoint)
...
class MakeFaceArray:
  def __init__(self, obj):
    ...
    facelist = [ (baseobj.Object, baseobj.SubElementNames) for baseobj in selobj[1:]]

    obj.addProperty("App::PropertyDistance","Offset","Parameters","Offset for Array").Offset = 0.0
    obj.addProperty("App::PropertyLinkSub", "Base", "Parameters", "The base object that will be dupicated").Base = (selobj[0].Object, selobj[0].SubElementNames)
    obj.addProperty("App::PropertyLinkSubList", "Faces", "Parameters", "Linked Faces").Faces = facelist
    obj.addProperty("App::PropertyLinkSubList", "SkipEdges", "Parameters", "Skipped Linked Edges")
    obj.addProperty("App::PropertyBool","Fuse","Parameters","Specifies, if array should be fused").Fuse = False
    obj.Proxy = self
  ...
  def execute(self, fp):
    ...
    linked_faces = [element[0].Shape.getElement(element[1][i]) for element in fp.Faces for i in range(len(element[1]))]
    if fp.SkipEdges:
      skip_edges = [element[0].Shape.getElement(element[1][i]) for element in fp.SkipEdges for i in range(len(element[1]))]
    else :
      skip_edges = []

    face_array = faceArray( base, base_face, linked_faces, skip_edges, -fp.Offset.Value)
...
def makeFaceArray():
    array = FreeCAD.ActiveDocument.addObject("Part::FeaturePython","FaceArray")
    MakeFaceArray(array)
