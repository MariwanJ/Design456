...
def ochecking_BOP(o):
    if hasattr(o, 'Shape'):
        chks = ocheckBOP(o.Shape)
        if chks is not True:
            oi_sayerr('shape \''+o.Name+'\' \'' +
                     omk_str(o.Label)+'\' is INVALID!\n')
                ...
                if len(o.Shape.Shells) > 0:
                    for sh in o.Shape.Shells:
                        try:
                            ...
                            oi_sayerr(omk_str(o.Label)+'.shell errors:')
                            oi_sayw(omk_str(sys.exc_info()[1]))
        else:
            oi_say('shape \''+o.Name+'\' \''+omk_str(o.Label)+'\' is valid\n')
##


def ocheck_TypeId_RH():
    try:
                ...
                for o in sel:
                    if hasattr(o, "Shape"):
                        if '.[compsolid]' in o.Label or '.[solid]' in o.Label or '.[shell]' in o.Label\
                            ...
                            o.Label = omk_str(o.Label).replace('.[solid]', '').replace('.[shell]', '').replace('.[compsolid]', '').replace('.[compound]', '')\
                            ...
                            if len(o.Shape.Solids) == 0:
                                oi_sayerr(omk_str(o.Label) +
                                         ' object is a NON Solid')
                            if len(o.Shape.CompSolids) > 0:
                                oi_say(
                                    omk_str(o.Label)+' CompSolids object(s) NBR : '+str(len(o.Shape.CompSolids)))
                                if '.[compsolid]' not in o.Label and '.[solid]' not in o.Label and '.[shell]' not in o.Label\
                                    ...
                                    o.Label = omk_str(o.Label)+'.[compsolid]'
                            if len(o.Shape.Compounds) > 0:
                                oi_say(
                                    omk_str(o.Label)+' Compound object(s) NBR : '+str(len(o.Shape.Compounds)))
                                if '.[compound]' not in o.Label and '.[solid]' not in o.Label and '.[shell]' not in o.Label:
                                    o.Label = omk_str(o.Label)+'.[compound]'
                            if len(o.Shape.Solids) > 0:
                                oi_say(
                                    omk_str(o.Label)+' Solid object(s) NBR : '+str(len(o.Shape.Solids)))
                                solids += omk_str(o.Label)+'<br>'
                                if '.[solid]' not in o.Label and '.[compsolid]' not in o.Label and '.[compound]' not in o.Label:
                                    o.Label = omk_str(o.Label)+'.[solid]'
                            else:

                                non_solids += omk_str(o.Label)+'<br>'
                            if len(o.Shape.Shells) > 0:
                                oi_say(
                                    omk_str(o.Label)+' Shell object(s) NBR : '+str(len(o.Shape.Shells)))
                                if '.[shell]' not in o.Label and '.[solid]' not in o.Label and '.[compsolid]' not in o.Label and '.[compound]' not in o.Label:
                                    o.Label = omk_str(o.Label)+'.[shell]'
                            if len(o.Shape.Faces) > 0:
                                oi_say(
                                    omk_str(o.Label)+' Face object(s) NBR : '+str(len(o.Shape.Faces)))
                                if '.[compsolid]' not in o.Label and '.[solid]' not in o.Label and '.[shell]' not in o.Label\
                                        and '.[compound]' not in o.Label and '.[face]' not in o.Label and '.[edge]' not in o.Label:
                                    o.Label = omk_str(o.Label)+'.[face]'
                            if len(o.Shape.Wires) > 0:
                                oi_say(
                                    omk_str(o.Label)+' Wire object(s) NBR : '+str(len(o.Shape.Wires)))
                                if '.[compsolid]' not in o.Label and '.[solid]' not in o.Label and '.[shell]' not in o.Label\
                                        and '.[compound]' not in o.Label and '.[face]' not in o.Label and '.[wire]' not in o.Label:
                                    o.Label = omk_str(o.Label)+'.[wire]'
                            if len(o.Shape.Edges) > 0:
                                oi_say(
                                    omk_str(o.Label)+' Edge object(s) NBR : '+str(len(o.Shape.Edges)))
                                if '.[compsolid]' not in o.Label and '.[solid]' not in o.Label and '.[shell]' not in o.Label\
                                        and '.[compound]' not in o.Label and '.[face]' not in o.Label and '.[edge]' not in o.Label and '.[wire]' not in o.Label:
                                    o.Label = omk_str(o.Label)+'.[edge]'
                            if len(o.Shape.Vertexes) > 0:
                                oi_say(
                                    omk_str(o.Label)+' Vertex object(s) NBR : '+str(len(o.Shape.Vertexes)))
                                if '.[compsolid]' not in o.Label and '.[solid]' not in o.Label and '.[shell]' not in o.Label\
                                        and '.[compound]' not in o.Label and '.[face]' not in o.Label and '.[edge]' not in o.Label\
                                        and '.[wire]' not in o.Label and '.[vertex]' not in o.Label:
                                    o.Label = omk_str(o.Label)+'.[vertex]'
                    else:
                        App.Console.PrintWarning(
        ...
        App.Console.PrintError("'ocheck_TypeId_RH' Failed. "
                               "{err}\n".format(err=str(err)))
...
def oclear_all_RH():
    try:
        ...
        rh_edges_to_connect = []
        created_faces = []
        oRHDockWidget.ui.TE_Edges.setPlainText("")
        rh_faces = []
        rh_faces_names = []
        rh_faces_indexes = []
        created_faces = []
        oRHDockWidget.ui.TE_Faces.setPlainText("")
        rh_obj = []
        rh_obj_name = []
        oRHDockWidget.ui.Edge_Nbr.setText("0")
        oRHDockWidget.ui.Face_Nbr.setText("0")
        oRHDockWidget.ui.Obj_Nbr.setText("0")
        oRHDockWidget.ui.Obj_Nbr_2.setText("0")
    except Exception as err:
        App.Console.PrintError("'oclear_all_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def orefine_parametric_RH():
    try:
        ...
        App.Console.PrintError("'orefine_parametric_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def orefine_RH():
    try:
        ...
        docG = Gui.ActiveDocument
        sel = Gui.Selection.getSelection()
        App.ActiveDocument.openTransaction(translate("Design456", "orefine_RH"))
        if len(sel):
            for o in sel:
                if hasattr(o, "Shape"):
        ...
        App.Console.PrintError("'orefine_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def oedges_clear_RH():
    try:
        ...
        App.Console.PrintError("'oedges_clear_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def ofaces_clear_RH():
    try:
        ...
        App.Console.PrintError("'ofaces_clear_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def omerge_selected_faces_RH():
    """merging Faces of selected shapes"""
    try:
        App.ActiveDocument.openTransaction(
            translate("Design456", "omerge_selected_faces_RH"))
        global rh_edges, rh_faces, rh_obj
        global rh_edges_names, rh_faces_names, rh_obj_name
        global created_faces, rh_faces_indexes, rh_edges_to_connect
        ...
        if len(af_faces) > 0:
            ...
            try:
                ...
                return
            if oRHDockWidget.ui.checkBox_Refine.isChecked():
                try:
        ...
        App.Console.PrintError("'omerge_selected_faces_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def osetTolerance():
    ...
    try:
        ...
        App.Console.PrintError("'osetTolerance' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def omerge_faces_from_selected_objects_RH(refobj=None):
    """merging Faces of selected shapes"""
    try:
        faces = []
        doc = App.ActiveDocument
        docG = Gui.ActiveDocument
        App.ActiveDocument.openTransaction(
            translate("Design456", "omerge_faces_from_selected_objects_RH"))
        _test = None
        sel = Gui.Selection.getSelection()
        if len(sel):
            for o in sel:
            ...
            if oRHDockWidget.ui.checkBox_keep_original.isChecked():
                for o in sel:
                    docG.getObject(o.Name).Visibility = False
            else:
                for o in sel:
        ...
        App.Console.PrintError("'omerge_faces_from_selected_objects_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def oedges_confirmed_RH():
    try:
        ...
        docG = Gui.ActiveDocument
        App.ActiveDocument.openTransaction(
            translate("Design456", "oedges_confirmed_RH"))
        en = None
        ...
        if len(selEx):
                    ...
                    if 'Edge' in selEdge.SubElementNames[i]:
                        ...
                        if not edge_in_list:
                            ...
                            if (e.isClosed()):
                                ...
                                oi_say('face created from closed edge')
                                if oRHDockWidget.ui.checkBox_keep_faces.isChecked():
            ...
            if len(rh_edges_to_connect) > 0:
                try:
                    ...
                    created_faces.append(cf)
                    oi_say('face created from open edges')
                    if oRHDockWidget.ui.checkBox_keep_faces.isChecked():
                        Part.show(cf)
                        doc.ActiveObject.Label = 'Face'
                        docG.ActiveObject.Visibility = False
            ...
            for e in rh_edges_names:
                e_list = e_list+str(e)+'\n'
            oRHDockWidget.ui.TE_Edges.setPlainText(e_list)
            oRHDockWidget.ui.Edge_Nbr.setText(str(len(rh_edges)))
            unique_obj = set(rh_obj)
            unique_obj_count = len(unique_obj)
            oRHDockWidget.ui.Obj_Nbr.setText(str(unique_obj_count))
            for ob in App.ActiveDocument.Objects:
                Gui.Selection.removeSelection(ob)

        App.ActiveDocument.commitTransaction()  # undo reg.
    except Exception as err:
        App.Console.PrintError("'oedges_confirmed_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def ofaces_confirmed_RH():
    try:
        ...
        doc = App.ActiveDocument
        App.ActiveDocument.openTransaction(
            translate("Design456", "ofaces_confirmed_RH"))
        selEx = Gui.Selection.getSelectionEx()
        ...
        if len(selEx):
            ...
            for f in rh_faces_names:
                f_list = f_list+str(f)+'\n'
            oRHDockWidget.ui.TE_Faces.setPlainText(f_list)

            oRHDockWidget.ui.Face_Nbr.setText(str(len(rh_faces)))
            unique_obj = set(rh_obj)
            unique_obj_count = len(unique_obj)
            oRHDockWidget.ui.Obj_Nbr_2.setText(str(unique_obj_count))
            for ob in App.ActiveDocument.Objects:
                Gui.Selection.removeSelection(ob)
            App.ActiveDocument.commitTransaction()  # undo reg.
    except Exception as err:
        App.Console.PrintError("'ofaces_confirmed_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def oremoveHoles_RH():
    try:
        ...
        global force_recompute, invert
        App.ActiveDocument.openTransaction(
                translate("Design456", "oremoveHoles_RH"))
        _test = None
        doc=App.ActiveDocument
        docG = Gui.ActiveDocument
        ...
        if unique_obj_count == 1:
            ...
            oi_sayerr('select only one object')
            
        App.ActiveDocument.commitTransaction()  # undo reg.

    except Exception as err:
        App.Console.PrintError("'oremoveHoles_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def oremoveFaces_RH():
    try:
        ...
        App.ActiveDocument.openTransaction(
            translate("Design456", "oremoveFaces_RH"))
        global rh_edges, rh_faces, rh_obj
        global rh_edges_names, rh_faces_names, rh_obj_name
        global created_faces, rh_faces_indexes, rh_edges_to_connect
        ...
        App.Console.PrintError("'oremoveFaces_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def oaddFaces_RH():
    try:
        App.ActiveDocument.openTransaction(
            translate("Design456", "oaddFaces_RH"))
        global rh_edges, rh_faces, rh_obj
        global rh_edges_names, rh_faces_names, rh_obj_name
        global created_faces, rh_faces_indexes, rh_edges_to_connect
        global force_recompute, invert
        ...
        App.Console.PrintError("'oaddFaces_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def ooffsetFaces_RH():
    try:
        App.ActiveDocument.openTransaction(
            translate("Design456", "ooffsetFaces_RH"))
        global rh_edges, rh_faces, rh_obj
        global rh_edges_names, rh_faces_names, rh_obj_name
        global created_faces, rh_faces_indexes, rh_edges_to_connect
        global force_recompute, invert
        ...
        if len(rh_faces) > 0:
            for f in rh_faces:
                ...
                offset_dir = oRHDockWidget.ui.offset_input.text().split(':')
                if len(offset_dir) > 1:
                    ...
                    if 'x' in offset_dir[1]:
                        ...
                        oi_sayerr('direction not inserted, using norm to face')
                        norm = f.normalAt(0, 0)
                        oRHDockWidget.ui.offset_input.setText(str(offset)+':n')
                else:
                    offset = float(oRHDockWidget.ui.offset_input.text())
                    oi_sayerr('direction not inserted, using norm to face')
                    norm = f.normalAt(0, 0)
                    oRHDockWidget.ui.offset_input.setText(str(offset)+':n')
        ...
        App.Console.PrintError("'ooffsetFaces_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def ooffsetEdges_RH():
    try:
        App.ActiveDocument.openTransaction(
            translate("Design456", "ooffsetEdges_RH"))
        global rh_edges, rh_faces, rh_obj
        global rh_edges_names, rh_faces_names, rh_obj_name
        global created_faces, rh_faces_indexes, rh_edges_to_connect
        global force_recompute, invert
            ...
            for e in rh_edges:
                ...
                if len(offset_dir) > 1:
                    ...
                    if 'x' in offset_dir[1]:
                        ...
                        oi_sayerr('direction not inserted, using z axis')
                        norm = App.Vector(0, 0, 1)
                else:
                    offset = float(oRHDockWidget.ui.offset_input.text())
                    oi_sayerr('direction not inserted, using z axis')
                    norm = App.Vector(0, 0, 1)
                    oRHDockWidget.ui.offset_input.setText(str(offset)+':x')
                fname = doc.ActiveObject.Name
                s = doc.ActiveObject.Shape.copy()
                s.translate(norm*offset)
                doc.removeObject(fname)
        ...
        App.Console.PrintError("'ooffsetEdges_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def oremovesubtree(objs):
    try:
        App.ActiveDocument.openTransaction(
            translate("Design456", "oremovesubtree"))
        ...
        App.Console.PrintError("'oremovesubtree' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def ocleaningFaces_RH():
    """merge two faces"""
    try:
        
        App.ActiveDocument.openTransaction(
            translate("Design456", "ocleaningFaces_RH"))
        global force_recompute, invert
        myshape = None
        oi_sayw('merging faces')
        doc = App.ActiveDocument
        docG = Gui.ActiveDocument
        selEx = Gui.Selection.getSelectionEx()
        fcs = []
        ...
        if len(selEx) > 0:
            ...
            omerge_faces_from_selected_objects_RH(myshape)
        App.ActiveDocument.commitTransaction()  # undo reg.
    except Exception as err:
        App.Console.PrintError("'ocleaningFaces_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def omakeEdge_RH():
    try:
        App.ActiveDocument.openTransaction(
            translate("Design456", "omakeEdge_RH"))
        global force_recompute
        doc = App.ActiveDocument
        docG = Gui.ActiveDocument
        verts = []
        ...
        if len(verts) == 2:
            try:
                oi_say("try to create an Edge w/ makeLine")
                ce = Part.makeLine(verts[0].Point, verts[1].Point)
                Part.show(ce)
                del ce
                doc.ActiveObject.Label = "Edge"
            except:
                oi_sayerr("failed to create a Line")
        else:
            oi_sayerr("select only 2 Vertexes")
        for ob in App.ActiveDocument.Objects:
            Gui.Selection.removeSelection(ob)
        App.ActiveDocument.commitTransaction()  # undo reg.
    except Exception as err:
        App.Console.PrintError("'omakeEdge_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def oaddEdges_RH():
    try:
        App.ActiveDocument.openTransaction(
            translate("Design456", "oaddEdges_RH"))
        global rh_edges, rh_faces, rh_obj
        global rh_edges_names, rh_faces_names, rh_obj_name
        global created_faces, rh_faces_indexes, rh_edges_to_connect
        global force_recompute
        ...
        App.Console.PrintError("'oaddEdges_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def oPartDefeaturing_RH():
    try:
        ...
        App.Console.PrintError("'oPartDefeaturing_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def omakeSolidExpSTEP_RH():
    try:
        App.ActiveDocument.openTransaction(
            translate("Design456", "omakeSolidExpSTEP_RH"))
        doc = App.ActiveDocument
        docG = Gui.ActiveDocument
        if doc is not None:
            fname = doc.FileName
        ...
        App.Console.PrintError("'omakeSolidExpSTEP_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def oshape_Connect_RH():
    ...
    try:
        ...
        if len(sel) == 1:
            App.ActiveDocument.openTransaction(
                translate("Design456", "oshape_Connect_RH"))
        ...
        App.Console.PrintError("'oshape_Connect_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def osimplecopy_RH():
    try:
        App.ActiveDocument.openTransaction(
            translate("Design456", "osimplecopy_RH"))
        doc = App.ActiveDocument
        docG = Gui.ActiveDocument
        sel = Gui.Selection.getSelection()
        if len(sel):
            ...
            oi_sayerr('select object(s) with a Shape')
        App.ActiveDocument.commitTransaction()  # undo reg.
    except Exception as err:

        App.Console.PrintError("'osimplecopy_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
def oloop_edges_RH():
    try:
        ...
        App.Console.PrintError("'oloop_edges_RH' Failed. "
                               "{err}\n".format(err=str(err)))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
...
################################################################################################
        ...
        self.PB_AEdges.clicked.connect(oaddEdges_RH)
        ...
        self.PB_makeShell.clicked.connect(omerge_selected_faces_RH)
        pm = QtGui.QPixmap()
        ...
        self.PB_makeShell_2.clicked.connect(
            omerge_faces_from_selected_objects_RH)
        ...
        self.PB_check_TypeId.clicked.connect(ocheck_TypeId_RH)
        ...
        self.PB_Edges.clicked.connect(oedges_confirmed_RH)
        pm = QtGui.QPixmap()
        ...
        self.PB_LoopEdge.clicked.connect(oloop_edges_RH)
        ...
        self.PB_Faces.clicked.connect(ofaces_confirmed_RH)
        ...
        self.PB_RHoles.clicked.connect(oremoveHoles_RH)
        ...
        self.PB_RFaces.clicked.connect(oremoveFaces_RH)
        pm = QtGui.QPixmap()
        ...
        self.PB_AFaces.clicked.connect(oaddFaces_RH)
        ...
        self.PB_CFaces.clicked.connect(ocopyFaces_RH)
        ...
        self.PB_Refine.clicked.connect(orefine_RH)
        pm = QtGui.QPixmap()
        ...
        self.PB_RefineParametric.clicked.connect(orefine_parametric_RH)
        ...
        self.PB_showEdgeList.clicked.connect(oshowEdges_RH)
        ...
        self.PB_showFaceList.clicked.connect(oshowFaces_RH)
        ...
        self.PB_TFace.clicked.connect(ooffsetFaces_RH)
        pm = QtGui.QPixmap()
        ...
        self.PB_TEdge.clicked.connect(ooffsetEdges_RH)
        ...
        self.PB_makeEdge.clicked.connect(omakeEdge_RH)
        pm = QtGui.QPixmap()
        ...
        self.PB_expSTEP.clicked.connect(omakeSolidExpSTEP_RH)
        ...
        self.PB_PartDefeaturing.clicked.connect(oPartDefeaturing_RH)
        ...
        self.PB_CleaningFaces.clicked.connect(ocleaningFaces_RH)
        ...
        self.PB_checkS.clicked.connect(ocheckShape)
        ...
        self.PB_setTol.clicked.connect(osetTolerance)
        ...
        self.PB_Connect.clicked.connect(oshape_Connect_RH)
        self.PB_Connect.setToolTip("Connect")
        pm = QtGui.QPixmap()
        ...
        self.PB_SimpleCopy.clicked.connect(osimplecopy_RH)
        pm = QtGui.QPixmap()
        ...
        # self.PB_close.clicked.connect(oclose_RH)
        # pm = QtGui.QPixmap()
        ...
        # self.PB_left.clicked.connect(odock_left_RH)
        ...
        # self.PB_right.clicked.connect(odock_right_RH)
        if pt_osx:
            self.InfoLabel.setText("Select Edge(s)\nCmd+Click")
            self.label.setText("Select Face(s)\nCmd+Click")
################################################################################################
...
def oRH_singleInstance():
    ...
    for i in dw:
        ...
        if str(i.objectName()) == "oDefeaturingTools":
            return False
    return True
##
...
def odock_right_RH():
    ...
    if oRHDockWidget and cv is not None:
        dw = t.findChildren(QtGui.QDockWidget)
