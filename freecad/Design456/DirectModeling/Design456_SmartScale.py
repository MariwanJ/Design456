...
def smartlbl_callback(smartLine, obj, parentlink):
    ...
    try:
        App.ActiveDocument.openTransaction(
            translate("Design456", "SmartScale"))
        ...
        __shape = Part.getShape(
            cloneObj, '', needSubElement=False, refine=False)
...
class smartLines(wlin.Fr_Line_Widget):
    ...
    def __init__(self, vectors: List[App.Vector] = [], label: str = "", lineWidth=1, linkToParent=None):
        # Must be done first as described in fr_line_widget
...
class Design456_SmartScale:
    ...
    def getXYZdimOfSelectedObject(self, selected):
        ...
        try:
            ...
            # Create the lines
            self.smartInd.clear()
            self.smartInd.append(smartLines(
                vectors[0], ["{0:.2f}".format(lengths[0]),], 5, self))
            self.smartInd.append(smartLines(
                vectors[1], ["{0:.2f}".format(lengths[1]),], 5, self))
            self.smartInd.append(smartLines(
                vectors[2], ["{0:.2f}".format(lengths[2]),], 5, self))
